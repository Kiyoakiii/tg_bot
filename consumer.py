...
#Мы подключились к брокеру сообщений, находящемуся на локальном хосте
#connection = pika.BlockingConnection(pika.URLParameters('amqp://guest:guest@rabbit:5672/?name=Server%20connection'))
""" connection = connect(os.environ['AMQP_URL'], channel_number=2,)
channel = connection.channel()
channel.queue_declare(queue='rpc_queue_tg_4', durable=True) #Проверяем есть ли очередь с таким же названием
 """
api_key=''
api_secret =''
SYMBOL =''
...
async def callback(ch, method, props, body):
    print (' [*] Waiting for messages. To exit press CTRL+C')
    connection = await connect(os.environ['AMQP_URL'], channel_number=2,)
    channel = await connection.channel()
    channel.queue_declare(queue='rpc_queue_tg_4', durable=True) #Проверяем есть ли очередь с таким же названием
    channel.basic_qos(prefetch_count=1)#Равномерно распределяет нагрузку на консумеры. Пока не закончит выполнять обработку одного сообщения, не берет новое. 
    channel.basic_consume(queue='rpc_queue_tg_4',
                        on_message_callback = callback
                        )
    global id
    print(' body ==== ', body.decode())
    message = body.decode()
    api_key = json.loads(message).pop("api_key")
    api_secret = json.loads(message).pop("api_secret")
    SYMBOL = json.loads(message).pop("symbol")
    id = json.loads(message).pop("user_id")
    print(SYMBOL,' ', api_secret, ' ', api_key)
    print('id = ', id)
    print(" [x] Done")
#провекра-подтверждение обработали ли мы полность сообщение из очереди или нет 
    response = trading(symbol=SYMBOL, api_key=api_key, api_secret=api_secret, id =id)
...
async def trading(symbol, api_key, api_secret, id):
    ...
    def trade(count_of_trades):
        ...
        while current_count < count_of_trades:
            print(sell, buy)
            if sell == True: 
                test_sequences, features, label_encoder = dataset()
            ...
            if buy == True:
                ...
                torg = (balance_current[0]/balance[0]-1)*100
                current_count +=1
