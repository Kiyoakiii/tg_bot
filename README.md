...
Обязанности:
Королев - Предложил идею + во время проекта назначал обязанности другим сокомандникам. Все что касается ML части сделал я + помогал ребятам с их задачками. Реализовал тг бота __init__.py для случая если деплой не удастся сделать 
Захаров - Решил проблему сбора данных (Get data) + 
Никулин - Алгоритм торговли, то есть функция ```trading``` в ```cosumer```
Марков - Реализовал телеграм бота (bot.py) + Docker. 
